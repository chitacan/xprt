...
module.exports = async () => {
  try {
    ...
    const {org} = await prompt({
      type: 'list',
      message: 'select organization to run export your data',
      name: 'org',
      choices: orgs
    })
    ...
    const prTotal = authored.filter(d => d.is_pr).length
    for (const [i, {is_pr, owner, repo, number}] of authored.entries()) {
      if (is_pr) {
        authoredLog.info('fetching pr details %d/%d', i, prTotal)
        const {data: {commits, additions, deletions, changed_files}} = await github.pullRequests.get({owner, repo, number})
        authored[i] = {...authored[i], commits, additions, deletions, changed_files}
      }
    }
    ...
    outputJsonSync(
      'github.json',
      {created: new Date(), authored, commented, reviewed},
      {spaces: 2}
    )
